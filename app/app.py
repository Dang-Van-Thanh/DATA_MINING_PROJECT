...
def show_home():
    ...
    if df is not None:
        ...
        with col1:
            ...
            st.container(height=120).metric("Tổng số khách hàng", f"{df.shape[0]:,}")
            st.markdown('</div>', unsafe_allow_html=True)
        
        with col2:
            st.markdown('<div class="metric-card">', unsafe_allow_html=True)
            config = load_config()
            feature_count = len(config['features']['numeric_cols']) + len(config['features']['categorical_cols'])
            st.container(height=120).metric("Số features", feature_count)
            st.markdown('</div>', unsafe_allow_html=True)
        
        with col3:
            success_rate = df['y'].mean() * 100
            st.markdown('<div class="metric-card">', unsafe_allow_html=True)
            st.container(height=120).metric("Tỷ lệ thành công", f"{success_rate:.2f}%")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col4:
            if results and 'supervised_learning' in results:
                ...
                st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                st.container(height=120).metric("Best Model", best_model)
                st.markdown('</div>', unsafe_allow_html=True)
...
def show_mining():
    ...
    with tab2:
        ...
        if profile_path.exists():
            profiles = pd.read_csv(profile_path, header=[0,1], index_col=0)
...
def show_semi():
    ...
    if semi_path.exists():
        ...
        if improvements:
            ...
            so với supervised-only. Hiệu quả rõ rệt hơn khi tỷ lệ nhãn trung bình-cao (≥20%), trong khi ở mức nhãn rất thấp (5%) hiệu suất có thể giảm.
            </div>
            """, unsafe_allow_html=True)
    else:
